...
# Заменяем весь блок с видео на изображение
old_block = '''<div class="about-image" id="aboutVideoWrapper">
...
new_block = '''<div class="about-image" style="position:relative;border-radius:20px;overflow:hidden;background:#1a1a2e;min-height:400px;">
...
if old_block in html:
    ...
    if 'aboutVideoWrapper' in html:
        print("   aboutVideoWrapper найден")
    if 'video-container' in html:
        print("   video-container найден")
    if 'aboutVideo' in html:
        print("   aboutVideo найден")

# Удаляем скрипт управления видео
old_script_pattern = r'<script>\n\(function\(\).*?</script>'
html = re.sub(old_script_pattern, '', html, flags=re.DOTALL)
print("✅ Скрипт управления видео удалён")

# Убираем autoplay и loop из всех video тегов (на всякий случай)
...
print("🎉 Готово! Видео заменено на изображение")
